...
try {
  ...
  execSync(
    `SERVERLESS_INDEX_NAME=${SERVERLESS_INDEX_NAME} ASSISTANT_NAME=${ASSISTANT_NAME} TEST_FILE=${TEST_FILE} TEST_ENV=${TEST_ENV} jest src/integration/assistant-data -c jest.config.integration-node.js --runInBand --bail`,
    { stdio: 'inherit' }
  );
} finally {
